              ...
              if (length && length > 2 && data[0] == '4' && data[1] == '2') {
                ...
                if (s != "") {
                  ...
                  if (event == "telemetry") {
                    ...
                    vector<int> a_lanes;
                    int all_lanes[3] = {0, 1, 2};

                    for (int i1 = 0; i1 < 3; ++i1) {
                      a_lanes.push_back(all_lanes[i1]);
                    }
                    ...
                    for (int m = 0; m < sensor_fusion.size(); ++m) {
                      float d = sensor_fusion[m][6];
                      double vx = sensor_fusion[m][3];
                      double vy = sensor_fusion[m][4];
                      ...
                      for (int i = 0; i < 3; ++i) {
                        if ((check_car_s > car_s && (check_car_s - car_s) < 30) ||
                                (all_lanes[i] != lane && check_car_s < car_s && (car_s - check_car_s) < 5)) {
                          if (d < (2 + 4 * all_lanes[i] + 2) && d > (2 + 4 * all_lanes[i] - 2)) {
                            a_lanes.erase(std::remove(a_lanes.begin(), a_lanes.end(), all_lanes[i]), a_lanes.end());
                          }
                        }
                    ...
                    if (std::find(a_lanes.begin(), a_lanes.end(), lane) != a_lanes.end()) {
                      // current lane is available
                      if (ref_vel < 49.5) {
                        ref_vel += .224;
                      }
                    } else {

                      if (std::find(a_lanes.begin(), a_lanes.end(), lane - 1) != a_lanes.end()) {
                        lane -= 1;
                      } else if (std::find(a_lanes.begin(), a_lanes.end(), lane + 1) != a_lanes.end()) {
                        lane += 1;
                      } else {
                        ref_vel -= .224;
                      }
                    ...
                    // use last 2 previous points for smooth start
                    if (prev_size < 2) {
                    ...
                    for (int l = 0; l <= 50 - previous_path_x.size(); ++l) {
                      ...
                      x_add_on = x_point;  // increase x_add_on each loop
